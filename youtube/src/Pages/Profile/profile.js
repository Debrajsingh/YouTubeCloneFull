...
const Profile = ({ sideNavbar }) => {
  ...
  const fetchProfileData = async () => {
    axios
      .get(`http://localhost:4000/api/68b8a48c66ce29bf6ca72e0a/channel`)
      .then((response) => {
                ...
                <Link to={`/watch/${item._id}`} className="profileVideos_block">
                  <div className="profileVideos_block_thumbnail">
                    <img
                      src={item?.thumbnail}
                      alt=""
                      className="profileVideos_block_thumbnail_img"
                    />
                  </div>
                  <div className="profileVideos_block_detail">
                    <div className="profileVideos_block_detail_name">{item?.title}
                    </div>
                    <div className="profileVideos_block_detail_about">
